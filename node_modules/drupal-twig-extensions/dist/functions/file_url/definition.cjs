...
function fileUrl(config, uri) {
  ...
  const path = `${uri}`; // This regex matches against uri schemes (e.g. 'public://', 'https://'). It
  // is copied from \Drupal\Core\StreamWrapper\StreamWrapperManager::getScheme()

  const scheme = /^([\w-]+):\/\//; // If the uri includes a streamWrapper scheme, replace it.
...
function configInit(state, config) {
  ...
  state.baseUrl = '/'; // The default Drupal streamWrapper paths.
  ...
  if (Object.prototype.hasOwnProperty.call(config, 'streamWrapper')) {
    ...
    state.streamWrapper = { ...state.streamWrapper,
